...
export default function CardGameStep({ speakingQs, kanjiQs, onBack, onRestart }: Props) {
  const level = useSelector((state: RootState) => state.course.level) ?? "";
  const cards = useMemo(() => buildCards(speakingQs, kanjiQs), [speakingQs, kanjiQs]);
      ...
      <main className="landing-shell bg-sakura-glow px-6 py-10 md:px-10 md:py-14">
        <section className="landing-card mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-3xl flex-col justify-between rounded-[36px] border border-white/60 bg-white/65 p-8 shadow-sakura backdrop-blur md:p-12">
          ...
          <div
            ...
            style={{ height: "22rem", opacity: visible ? 1 : 0, transition: "opacity 0.15s ease" }}
          ...
          >
            <div className={`flip-inner${flipped ? " flipped" : ""}`}>
              ...
              <div className="flip-face flip-front">
                ...
                <p className="mt-2 text-3xl font-bold text-sakura-iris">{card.boldWord}</p>
                <p className="mt-4 text-xs text-sakura-iris/60">タップして答えを見る</p>
              </div>
              ...
              <div className="flip-face flip-back">
                ...
                <p className="text-5xl font-bold text-sakura-dusk">{card.answer}</p>
                <p className="mt-2 text-center text-base text-sakura-iris">{card.boldWord}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <Button
              ...
              sx={(theme) => ({
                ...
                py: "0.6rem !important",
                "&:hover": { background: `${theme.palette.sakura.mist} !important`, borderColor: "transparent" },
                "&.Mui-disabled": { opacity: 0.3, background: "rgba(255,255,255,0.2) !important" },
              })}
            >
              ←
            </Button>
