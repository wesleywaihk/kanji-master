                ...
                {speakingQs.map((q, i) => (
                  <li key={i} className="flex gap-2 text-base text-sakura-dusk">
                    ...
                    <span className="min-w-0 flex-1 break-words">
                      <span dangerouslySetInnerHTML={{ __html: buildSentence(q.question, q.kanji) }} />
                      <span className="ml-3 font-semibold text-sakura-iris">→ {q.kana}</span>
                    </span>
                  </li>
                ))}
              </ol>
              ...
              <ol className="space-y-3">
                {kanjiQs.map((q, i) => (
                  <li key={i} className="flex gap-2 text-base text-sakura-dusk">
                    <span className="w-12 shrink-0 font-medium">{toFullwidth(i + 1)}．</span>
                    <span className="min-w-0 flex-1 break-words">
                      <span dangerouslySetInnerHTML={{ __html: buildSentence(q.question, q.kana) }} />
                      <span className="ml-3 font-semibold text-sakura-iris">→ {q.kanji}</span>
                    </span>
                  </li>
                ))}
              </ol>
